...
PROCEDURE WriteDate(VAR FOut: TEXT; VAR Res: Date); {Вывод даты в файл}
PROCEDURE ReadDate(VAR FIn: TEXT; VAR Res: Date); {Чтение даты из файла}
PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month); {Чтение месяца из файла}
PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month); {Вывод месяца в файл}
FUNCTION Less(VAR D1, D2: Date): BOOLEAN; {Сравнение дат}
PROCEDURE CopyOut(VAR DateFile: FileOfDate); {Вывод файла дат в OUTPUT}

IMPLEMENTATION
PROCEDURE WriteDate(VAR FOut: TEXT; VAR Res: Date);
{Вывод месяца, затем числа в FOut}
BEGIN {WriteDate}
...
{Чтение трех последовательных букв как представление месяца}
VAR
  Ch1, Ch2, Ch3: CHAR;
BEGIN {ReadMonth}
...
{Вывод соответствующего месяцу текстового представления}
VAR
  Ch1, Ch2, Ch3: CHAR;
BEGIN {WriteMonth}
...
{Сравнение месяца, затем дня дат; возвращает TRUE, если D1 предшествует D2}
BEGIN {Less}
  IF D1.Mo < D2.Mo
  THEN
    Less := TRUE
...
{Копирование всего файла дат в OUTPUT в соответствующем текстовом представлении}
VAR
  VarDate: Date;
BEGIN {CopyOut}
